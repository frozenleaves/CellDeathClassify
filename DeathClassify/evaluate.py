...
def evaluate():
    ...
    dic_root_D = os.path.join(config.test_dir_dic_20x, 'D/')
    mcy_root_D = os.path.join(config.test_dir_mcy_20x, 'D/')
    dic_root_N = os.path.join(config.test_dir_dic_20x, 'N/')
    mcy_root_N = os.path.join(config.test_dir_mcy_20x, 'N/')


    dic_D = [os.path.join(dic_root_D, x) for x in os.listdir(dic_root_D)]
    mcy_D = [os.path.join(mcy_root_D, x) for x in os.listdir(mcy_root_D)]
    dic_N = [os.path.join(dic_root_N, x) for x in os.listdir(dic_root_N)]
    mcy_N = [os.path.join(mcy_root_N, x) for x in os.listdir(mcy_root_N)]
    step = 1
    predict_img = []
    for i in zip(dic_D, mcy_D):
    ...
    print(rets)
    print('all', len(rets))
    print('D', rets.count(0))
    print('N', rets.count(1))
