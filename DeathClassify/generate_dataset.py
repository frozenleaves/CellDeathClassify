...
raw = r"C:\Users\wjq\Desktop\json-文件和图象"

def get_path(root_dir=raw):
    dataset_dir = []
...
def generate(dataset_dir, cellfilter: int = 10, save_train_dataset="train_classification_dataset"):
    ...
    json_path = glob.glob(dataset_dir + '\\*-GT.json')[0]
    assert len(os.listdir(dic_dir)) == len(os.listdir(mcy_dir))
    save_dir = os.path.join(dataset_dir, save_train_dataset)
    ...
    for data in generator.generate():
        if max(data.image_mcy.shape) < cellfilter:
            ...
            if 0 in data.image_mcy.shape:
                # print('filter')
                continue
            tifffile.imwrite(dic, data.image_dic)
            # print(dic)
            tifffile.imwrite(mcy, data.image_mcy)
            # print(mcy)
...
if __name__ == '__main__':
    generate(r'G:\paper\evaluate_data\copy_of_1_xy10')
